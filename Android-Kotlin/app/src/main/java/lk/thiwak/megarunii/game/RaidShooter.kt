...
class RaidShooter(val context: Context, private val gameConfig:JSONObject) : Game(gameConfig, context) {


    var previousGiftTimeMilli = System.currentTimeMillis()
    ...
    private fun calScoreCurrentRound(): Int{
        ...
        for (i in 1..MAX_BLACK_ZONE){
            shotValues.add(BLACK_ZONE)
        }
        for (i in 1..MAX_RED_ZONE){
    ...
    fun calWaitingTime(): Long {
        ...
        waitingTimeMilli = (1000*waitingTime.sum()).toLong() + dynamicWaitingTime.toLong()

        val currentTimeStr = getCurrentTimeStr()
    ...
    fun askForTheGift() {
        ...
        previousGiftTimeMilli = System.currentTimeMillis()
